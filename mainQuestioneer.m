...
[class,~,D] = kmeans(dataTest,kClassRows,'OnlinePhase','on','Replicates',500);
...
errArray = zeros(length(nColsTest),3,4);
for j = 1:length(nColsTest)
...
[~, accuracyKmeans]         = trainClassifierQuestioneer(embeddingKmeansLabeled);
[~, accuracyMahalanobis]    = trainClassifierQuestioneer(embeddingMahalanobisLabeled);
