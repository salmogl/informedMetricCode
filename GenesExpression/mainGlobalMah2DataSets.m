...
load(loadPath);
dataStruct_CANDF    = dataStruct;
dataSet             = 'MSK';
loadPath            = addPathLoad(dataSet);
load(loadPath);
dataStruct_MSK      = dataStruct;
...
dataCenteredCANDF         = dataStruct_CANDF.genesData;
dataCenteredMSK           = dataStruct_MSK.genesData;
dataCenteredCANDF(7206,:) = []; % remove NaN

data        = [dataCenteredCANDF dataCenteredMSK];
month       = [dataStruct_CANDF.month ; dataStruct_MSK.month];
vitalStat   = [dataStruct_CANDF.vitalStat ; dataStruct_MSK.vitalStat];

[val,indMaxStd]     = sort(std(data,[],2),'descend');
data                = data(indMaxStd(2:201),:);
[M,N]               = size(data);
rank                = 5;
kClusters           = 19;%10:1:30;

for kk = 1:length(kClusters)

class               = kmeans(data,kClusters(kk),'MaxIter',1e6,'OnlinePhase','on','replicates',1e4);
[~,indSort]         = sort(class);
...
% groupsPca(Vpca1 > 0)= 2;
groupsPca           = kmeans(Vpca',2,'MaxIter',1e6,'OnlinePhase','on','replicates',100);
%%
...
groupsInf           = kmeans(Vinf',2,'MaxIter',1e6,'OnlinePhase','on','replicates',100);
%%
...
xlim([1 20])
ylabel('Normalized eigenvalues','Interpreter','Latex','FontSize',fSize)

%%
load('../MatFiles/pValVsK_15_1_40_var_1.mat')
...
figure;
plot(kClusters,log(pPcaAllK),'linewidth',2);
hold on
plot(kClusters,log(pInfAllK),'linewidth',2);
ylabel('p-Value [dB]','fontsize',15)
xlabel('K [Clusters]','fontsize',15)
xlim([10 30])
le = legend('$p_{LM}$','$p_{ILM}$');
