...
knn         = 104; %[45:5:75 82];%[10 15 20 25 30 35 40 45]; % 20 is the optimal
...
for mm = 1:length(knn)
    ...
    argsPca.knn         = knn(mm);
    argsInf.knn         = knn(mm);
    mDistPca            = PCA_Reconstruction_New(data,argsPca);
    ...
    groupsPca(Vpca > 0) = 2;
    %%
    groupsPca           = kmeans(embeddingPca,2,'MaxIter',1e6,'OnlinePhase','on','replicates',100);
    %%
...
[mDistClust,sumD(mm)] = PCA_Reconstruction_New(data,argsInf);
...
%%
groupsClust                   = kmeans(embeddingClust,2,'MaxIter',1e6,'OnlinePhase','on','replicates',100);
%%
...
% tsneRaw         = tsne(embeddingClust, clustLabels, no_dims, initial_dims, perplexity);
